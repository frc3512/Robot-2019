...
TrapezoidalMotionProfile::TrapezoidalMotionProfile(Constraints constraints,
    ...
    units::meter_t fullTrapezoidDist = cutoffDistBegin +
    ...
    if (fullSpeedDist < 0_m) {
        accelerationTime = units::math::sqrt(fullTrapezoidDist /
...
units::second_t TrapezoidalMotionProfile::TimeLeftUntil(
    units::meter_t target) const {
    ...
    units::meters_per_second_squared_t acceleration =
        m_constraints.maxAcceleration;
    units::meters_per_second_squared_t decceleration =
        -m_constraints.maxAcceleration;
    ...
    if (endAccel > 0_s) {
        deccelVelocity = units::math::sqrt(units::math::abs(
            velocity * velocity + 2 * acceleration * accelDist));
    } else {
        deccelVelocity = velocity;
    }

    units::meter_t deccelDist = deccelVelocity * endDeccel +
